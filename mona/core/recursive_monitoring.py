...
import psutil
from mona.core import scheduler, monitoring
...
class RecursiveMonitoring(threading.Thread):
    ...
    def __init__(self, pid: int, buffer: 'Buffer',
                 interval: float = 1, recursion_interval: float = 0.1) -> None:
        """
        Monitor a process and its children
    ...
    def run(self) -> None:
        ...
        monitor = monitoring.Monitoring(self.pid,
                                        self.interval,
                                        self.buffer)
    ...
    def monitor_children(self) -> None:
        ...
        for child_proc in children:
            if child_proc not in self.processes:
                ...
                monitor = monitoring.Monitoring(child_proc.pid, self.interval,
                                                self.buffer, True)
                threading.Thread(
